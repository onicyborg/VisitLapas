    ...
    <div class="card-header flex-wrap d-flex align-items-center gap-3">
      ...
      <div class="d-flex flex-wrap align-items-end gap-2 ms-auto">
        ...
        <div>
          ...
          <input type="date" id="f_to" class="form-control form-control-sm" value="{{ $to }}">
        </div>
        <div class="input-group input-group-sm" style="width: 260px;">
          <span class="input-group-text"><i class="bi bi-search"></i></span>
          <input type="text" id="f_search" class="form-control form-control-sm" placeholder="Ticket/Visitor/Inmate">
        </div>
        <div>
          <button type="button" id="btnExport" class="btn btn-success btn-sm"><i class="bi bi-file-earmark-excel"></i> Export Excel</button>
        </div>
      </div>
    </div>
    <div class="card-body">
...
function buildExportUrl(){
  ...
  const from = document.getElementById('f_from').value; if (from) params.set('from', from);
  const to = document.getElementById('f_to').value; if (to) params.set('to', to);
  const q = document.getElementById('f_search').value; if (q) params.set('q', q);
